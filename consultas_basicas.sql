...
-- Tiene sentido usar SELECT * en una etapa de exploración incial cuando se quiere conocer de manera general la estructura de la tabla. Por otro lado no conviene cuando la tabla tenga muchas columnas y filas porque consume más tiempo de procesamiento. También, si lo van a leer otros, siempre es más util y entendible nombrar las columnas.  
SELECT * FROM sales;
...
-- fecha_pedido, nombre_producto y cantidad_unidades
SELECT order_date AS fecha_pedido, product_id AS nombre_producto, quantity AS cantidad_unidades
FROM sales;
